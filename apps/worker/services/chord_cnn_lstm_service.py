...
class ChordCNNLSTMDetectorService:
    ...
    def is_available(self) -> bool:
        ...
        try:
            if not self.model_dir or not self.model_dir.exists():
                print("Chord-CNN-LSTM model directory not found")
            ...
            try:
                ...
                print("Chord-CNN-LSTM availability: True")
                return True
            except ImportError as e:
                print(f"Chord-CNN-LSTM import failed: {e}")
                self._available = False
                return False
            finally:
            ...
            print(f"Error checking Chord-CNN-LSTM availability: {e}")
            self._available = False
            return False
    ...
    def recognize_chords(self, file_path: str, chord_dict: str = 'submission', **kwargs) -> Dict[str, Any]:
        ...
        try:
            print(f"Running Chord-CNN-LSTM recognition on: {file_path} with chord_dict={chord_dict}")
            ...
            # Run chord recognition with absolute path
            chord_recognition(abs_file_path, temp_lab_path, chord_dict)
            ...
            if not chord_data:
                return {
                    "success": False,
                    "error": "No chords detected or lab file parsing failed",
                    "model_used": "chord-cnn-lstm",
                    "model_name": "Chord-CNN-LSTM",
                    "chord_dict": chord_dict,
                    "processing_time": time.time() - start_time
            ...
            print(f"Chord-CNN-LSTM recognition successful: {len(chord_data)} chords detected")
            ...
            if temp_lab_path and os.path.exists(temp_lab_path):
                try:
                    ...
                    print(f"Failed to clean up temporary lab file: {e}")
    ...
    def _parse_lab_file(self, lab_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"Error parsing lab file {lab_path}: {e}")
